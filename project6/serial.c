...
ISR(USART0_UDRE_vect)
{
	...
	CLEAR_BIT(UCSR0B, UDRIE0);
        ...
        // Send the current character, then advance the
        // circular buffer.
        UDR0 = *ptrOutputCharTail;

	ptrOutputCharTail = INC_CIRC_BUFFER_PTR(
                ptrOutputCharTail,
                zOutputChars, MAX_OUT_STR_SIZE);

        // If we’ve transferred the final character, leave the
        // interrupt disabled.
...
ISR(USART0_RX_vect)
{
	unsigned char status;
        ...
        // Fetch the status and the next received character.
        status = UCSR0A;
	*ptrInputCharHead = UDR0;
	...
	ptrInputCharHead = INC_CIRC_BUFFER_PTR(
                ptrInputCharHead, zInputChars,
                MAX_IN_STR_SIZE);

        // Check for a receive buffer overrun.
